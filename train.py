...
if __name__ == "__main__":
    ...
    bmt.init_distributed(seed=1003)#, gpu_list=gpu_list)
    print_rank(args.hyper_para)
    parse_hyper_para(args.hyper_para, config)


    cuda = torch.cuda.is_available()
    output_log(logger, "CUDA available: %s" % str(cuda))
    if not cuda and len(gpu_list) > 0:
        output_log(logger, "CUDA is not available but specific gpu id", level=logging.ERROR)
