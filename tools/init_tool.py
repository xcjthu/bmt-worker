...
def init_all(config, gpu_list, checkpoint, mode, *args, **params):
    ...
    lr_scheduler = get_scheduler(config.get("train", "scheduler"), optimizer, config.getfloat('train', 'learning_rate'), config.getint('train', 'warmup_steps'), config.getint('train', 'training_steps'), params["skip_step"])
